...
TEST(BignumHandleTest, ConstructorFromBIGNUM) {
	...
	jwt::helper::bignum_handle bn(original);
	ASSERT_NE(nullptr, bn.get());
...
TEST(BignumHandleTest, GetMethod) {
	jwt::helper::bignum_handle bn;
	const BIGNUM* ptr = bn.get();
	ASSERT_NE(nullptr, ptr);

	const BIGNUM* ptr2 = bn.get();
	EXPECT_EQ(ptr, ptr2);
}

TEST(BignumHandleTest, OperatorNot) {
	jwt::helper::bignum_handle bn_valid;
	EXPECT_FALSE(!bn_valid);

	jwt::helper::bignum_handle bn_source;
	jwt::helper::bignum_handle bn_dest(std::move(bn_source));
...
TEST(BignumHandleTest, DestructorCleansUp) {
	...
	{
		jwt::helper::bignum_handle handle(bn);
...
TEST(BignumHandleTest, LargeBIGNUM) {
	...
	jwt::helper::bignum_handle bn(large);
	EXPECT_FALSE(BN_is_zero(bn.get()));
}

TEST(BignumHandleTest, NullPointerThrows) {
	EXPECT_THROW({ jwt::helper::bignum_handle bn(nullptr); }, std::runtime_error);
}
...
TEST(BignumHandleTest, IntegrationWithRaw2bn) {
	...
	const BIGNUM* raw = bn_ptr.release();
	ASSERT_NE(nullptr, raw);

	jwt::helper::bignum_handle bn(raw);
	EXPECT_FALSE(BN_is_zero(bn.get()));
}
