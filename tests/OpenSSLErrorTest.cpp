...
EVP_PKEY *PEM_read_bio_PrivateKey(BIO *bp, EVP_PKEY **x, pem_password_cb *cb, void *u) {
    static EVP_PKEY*(*origMethod)(BIO *bp, EVP_PKEY **x, pem_password_cb *cb, void *u) = nullptr;
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "PEM_read_bio_PrivateKey");
...
EC_KEY *PEM_read_bio_EC_PUBKEY(BIO *bp, EC_KEY **x, pem_password_cb *cb, void *u) {
    ...
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "PEM_read_bio_EC_PUBKEY");
...
EC_KEY *PEM_read_bio_ECPrivateKey(BIO *bp, EC_KEY **x, pem_password_cb *cb, void *u) {
    static EC_KEY *(*origMethod)(BIO *bp, EC_KEY **x, pem_password_cb *cb, void *u) = nullptr;
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "PEM_read_bio_ECPrivateKey");
...
unsigned char *HMAC(const EVP_MD *evp_md, const void *key, int key_len,
                    ...
                    unsigned int *md_len) {
    static unsigned char *(*origMethod)(const EVP_MD *evp_md, const void *key, int key_len,
                    const unsigned char *d, size_t n, unsigned char *md,
                    unsigned int *md_len) = nullptr;
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "HMAC");
    bool fail = fail_HMAC & 1;
    fail_HMAC = fail_HMAC >> 1;
    if(fail) return nullptr;
    else return origMethod(evp_md, key, key_len, d, n, md, md_len);
}

EVP_MD_CTX *EVP_MD_CTX_new(void) {
    static EVP_MD_CTX *(*origMethod)(void) = nullptr;
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "EVP_MD_CTX_new");
...
int EVP_SignFinal(EVP_MD_CTX *ctx, unsigned char *md, unsigned int *s, EVP_PKEY *pkey) {
    ...
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "EVP_SignFinal");
    bool fail = fail_EVP_SignFinal & 1;
    fail_EVP_SignFinal = fail_EVP_SignFinal >> 1;
    if(fail) return 0;
    else return origMethod(ctx, md, s, pkey);
}

int EVP_VerifyFinal(EVP_MD_CTX *ctx, const unsigned char *sigbuf, unsigned int siglen, EVP_PKEY *pkey) {
    static int(*origMethod)(EVP_MD_CTX *ctx, const unsigned char *sigbuf, unsigned int siglen, EVP_PKEY *pkey) = nullptr;
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "EVP_VerifyFinal");
    bool fail = fail_EVP_VerifyFinal & 1;
    fail_EVP_VerifyFinal = fail_EVP_VerifyFinal >> 1;
    if(fail) return 0;
    else return origMethod(ctx, sigbuf, siglen, pkey);
}

int EC_KEY_check_key(const EC_KEY *key) {
    static int(*origMethod)(const EC_KEY *key) = nullptr;
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "EC_KEY_check_key");
    bool fail = fail_EC_KEY_check_key & 1;
    fail_EC_KEY_check_key = fail_EC_KEY_check_key >> 1;
    if(fail) return 0;
    else return origMethod(key);
}

ECDSA_SIG *ECDSA_SIG_new(void) {
    static ECDSA_SIG*(*origMethod)() = nullptr;
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "ECDSA_SIG_new");
...
ECDSA_SIG *ECDSA_do_sign(const unsigned char *dgst, int dgst_len, EC_KEY *eckey) {
    ...
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "ECDSA_do_sign");
    bool fail = fail_ECDSA_do_sign & 1;
    fail_ECDSA_do_sign = fail_ECDSA_do_sign >> 1;
    if(fail) return nullptr;
    else return origMethod(dgst, dgst_len, eckey);
}

struct rsa_st *EVP_PKEY_get1_RSA(EVP_PKEY *pkey) {
    static struct rsa_st *(*origMethod)(EVP_PKEY* pkey) = nullptr;
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "EVP_PKEY_get1_RSA");
...
TEST(OpenSSLErrorTest, ES256SignErrorCode) {
    ...
    std::vector<multitest_entry> mapping = {
        ...
        { &fail_ECDSA_do_sign, 1, jwt::error::signature_generation_error::ecdsa_do_sign_failed }
    };
